...
SCENARIOS: dict[str, ScenarioSpec] = {
    ...
    "bridge_partition": ScenarioSpec(
        ...
        node_fault_model=all_nodes_active,
        rounds=5,
        description="Nodes 1 and 2 both miss each other in round 0, so the system forms a bridge topology rather than a clean asymmetric loss. Node 0 still commits round 0 from its complete local view, while nodes 1 and 2 halt immediately on missing commit evidence and node 0 halts one round later once the bridge ambiguity reaches its local sound matrix.",
        expectation=ScenarioExpectation(
